...
void AudioMixer_init(void)
{
	...
	printf("...Creating playback thread.\n");
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	{
		printf("ERROR: Can't queue next sound.\n");
	}
...
static inline short short_add(short x, short y)
{
...
static void fillPlaybackBuffer(short *playbackBuffer, int size)
{
	...
	for (i = 0; i < MAX_SOUND_BITES; i++)
	{
		if (soundBites[i].pSound != NULL)
		{
			offset = soundBites[i].location;
			sampleLeft = soundBites[i].pSound->numSamples - offset;
			if (sampleLeft > size)
			{
				for (j = 0; j< size; j++)
					playbackBuffer[j] = short_add(playbackBuffer[j], soundBites[i].pSound->pData[j+offset]);
				soundBites[i].location += size;
			}
			else
			{
				for (j = 0; j< sampleLeft; j++)
					playbackBuffer[j] = short_add(playbackBuffer[j], soundBites[i].pSound->pData[j+offset]);
				soundBites[i].pSound = NULL;
				soundBites[i].location = 0;
			}
